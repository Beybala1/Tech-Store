...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="mb-0">@lang('messages.alt-sub-category') @lang('messages.create')</h3>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('alt-sub-category.update', $altSubCategory->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="alt_category_id" class="form-control" required>
                        ...
                        @foreach($altCategories as $altCategory)
                            <option value="{{ $altCategory->id }}" {{ $altCategory->id==$altSubCategory->alt_category_id ? 'selected' : '' }}>{{ $altCategory->title }}</option>
                        @endforeach
                    </select>
                </div>
                @foreach (lang() as $language)
                    <div class="tab-pane fade {{ $language->code === app()->getLocale() ? 'show active' : '' }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title[{{ $language->code }}]" class="form-control"
                                value="{{ $altSubCategory->translate($language->code)->title }}" id="basic-default-title"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="slug[{{ $language->code }}]" class="form-control"
                                value="{{ $altSubCategory->translate($language->code)->slug }}" id="basic-default-slug"
                                placeholder="@lang('messages.slug')" required>
                        </div>
                        <div class="pt-2">
                            <button type="submit" class="btn btn-primary me-sm-3 me-1">@lang('messages.store')</button>
                            <a href="{{ route('alt-category.index') }}" class="btn btn-label-secondary">@lang('messages.cancel')</a>
                        </div>
                    </div>
                @endforeach
            </div>
