...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="mb-0">@lang('messages.about') @lang('messages.create')</h3>
    </div>
    <div class="card-body">
        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <img class="rounded me-2" alt="200x200" width="200" id="test"
                                    src="{{ asset($about->image) }}" data-holder-rendered="true">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="tab-pane fade {{ $language->code === app()->getLocale() ? 'show active' : '' }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title[{{ $language->code }}]" class="form-control"
                                value="{{ $about->translate($language->code)->title }}" id="basic-default-title"
                                placeholder="@lang('messages.title')" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="basic-default-content">@lang('messages.content')</label>
                            <textarea name="description[{{ $language->code }}]" class="form-control" cols="20" rows="7" required>{{ $about->translate($language->code)->description }}</textarea>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" name="alt[{{ $language->code }}]" class="form-control"
                                value="{{ $about->translate($language->code)->alt }}" id="basic-default-alt"
                                placeholder="@lang('messages.alt')" required>
                        </div>
                        <div class="pt-2">
                            <button type="submit" class="btn btn-primary me-sm-3 me-1">@lang('messages.store')</button>
                            <a href="{{ route('about.index') }}" class="btn btn-label-secondary">@lang('messages.cancel')</a>
                        </div>
                    </div>
                @endforeach
            </div>
