...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="mb-0">@lang('messages.alt-category') @lang('messages.create')</h3>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('alt-category.update', $altCategory->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="category_id" class="form-control" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $category->id==$altCategory->category_id ? 'selected' : '' }}>{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
                @foreach (lang() as $language)
                    <div class="tab-pane fade {{ $language->code === app()->getLocale() ? 'show active' : '' }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="title[{{ $language->code }}]" class="form-control"
                                value="{{ $altCategory->translate($language->code)->title }}" id="basic-default-title"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="slug[{{ $language->code }}]" class="form-control"
                                value="{{ $altCategory->translate($language->code)->slug }}" id="basic-default-slug"
